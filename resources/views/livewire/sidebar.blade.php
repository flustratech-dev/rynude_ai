<div
    ...
    class="sidebar-card h-full w-full flex-1 min-h-0 flex flex-col bg-[#F9F8F6] dark:bg-claude-sidebar-dark font-claude-response"
>
    {{-- ========== EXPANDED MODE ========== --}}
    <div x-show="open" x-cloak class="h-full w-full flex-1 min-h-0 flex flex-col" x-data="{ searchOpen: false, searchQuery: '' }">
        ...
        <div class="flex items-center justify-between px-3 py-2.5 mt-0.5">
            ...
            <div class="flex items-center gap-1">
                <button
                    ...
                    :class="searchOpen ? 'text-[#D97757]' : 'text-gray-500 dark:text-stone-200 hover:text-[#2D2825] dark:hover:text-stone-200'"
                    title="Search chats"
                >
                ...
                <button
                    ...
                    class="p-1 text-gray-500 dark:text-stone-200 hover:text-[#2D2825] dark:hover:text-stone-200 transition-colors group"
                >
        ...
        <div class="px-2 mt-1">
            <button @click="activePanel = null; window.dispatchEvent(new CustomEvent('close-artifact-panel')); if (window.innerWidth < 768) { sidebarOpen = false; open = false; }; window.dispatchEvent(new CustomEvent('newChat')); window.history.pushState({}, '', '{{ route('chat') }}');" class="w-full flex items-center gap-2.5 px-2 py-1.5 rounded-lg text-[13px] text-[#2D2825] dark:text-stone-300 hover:bg-[#EAE9E5]/60 dark:hover:bg-[#3A3A38] transition-colors group">
                <div class="flex items-center justify-center rounded-full size-[1.4rem] -mx-[0.2rem] border border-[#DCD7D2] dark:border-stone-600 bg-[#EAE6E1]/40 dark:bg-stone-700/30 transition-all ease-in-out group-hover:-rotate-3 group-hover:scale-110 group-active:rotate-6 group-active:scale-[0.98]">
                    <svg class="w-[14px] h-[14px] text-gray-600 dark:text-stone-200 group-hover:text-[#2D2825] dark:group-hover:text-white transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
        ...
        <div class="flex-1 min-h-0 overflow-y-auto custom-scrollbar px-2">
            ...
            >
                <svg class="w-[18px] h-[18px] flex-shrink-0 dark:text-stone-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        {{-- Products --}}
        <div class="mt-4" x-data="{ productsOpen: true }">
            <div class="px-2 py-1">
                <button @click="productsOpen = !productsOpen" class="flex items-center gap-1.5 text-[12px] font-medium text-gray-500 dark:text-stone-400 hover:text-gray-700 dark:hover:text-stone-300 transition-colors">
                    <span>Products</span>
                    <svg class="w-3 h-3 dark:text-stone-200 transition-transform duration-200" :class="productsOpen ? 'rotate-90' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
                </button>
            </div>
        ...
        {{-- Recents --}}
        <div class="mt-4" x-data="sidebarRecentsState()" x-init="init()">
            <div class="flex items-center justify-between px-2 py-1">
                <button @click="recentsOpen = !recentsOpen" class="flex items-center gap-1.5 text-[12px] font-medium text-gray-500 dark:text-stone-400 hover:text-gray-700 dark:hover:text-stone-300 transition-colors">
                    <span>Recents</span>
                    <svg class="w-3 h-3 dark:text-stone-200 transition-transform duration-200" :class="recentsOpen ? 'rotate-90' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
                </button>
                <button @click="activePanel = activePanel === 'chats' ? null : 'chats'; window.dispatchEvent(new CustomEvent('close-artifact-panel'))" class="text-gray-400 dark:text-stone-500 hover:text-gray-600 dark:hover:text-stone-300 transition-colors" title="Manage chats">
                    <svg class="w-[14px] h-[14px] dark:text-stone-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                ...
                <template x-for="(items, groupName) in groupedRecents" :key="groupName">
                    <div>
                        ...
                        <template x-for="c in items" :key="c.id">
                            <div class="relative group flex items-center w-full rounded-lg transition-all duration-200 ease-in-out"
        ...
        {{-- Bottom Profile --}}
        <div class="profile-footer-container border-t border-claude-border-light dark:border-claude-border-dark px-2 py-2 relative bg-[#F9F8F6] dark:bg-claude-sidebar-dark">
            @auth
                <div x-data="{ profileMenuOpen: false }">
                    <div
                        ...
                        class="absolute bottom-full left-2 mb-2.5 w-[260px] max-h-[60vh] overflow-y-auto overflow-x-hidden custom-scrollbar bg-[#FCFBFA] dark:bg-[#2C2C2C] border border-gray-200 dark:border-stone-700 rounded-xl shadow-xl z-50 py-1"
                        style="display: none;"
                    >
                        <div class="text-[13px] text-gray-700 dark:text-stone-200 font-semibold px-3 py-1 mb-0.5 border-b border-gray-100 dark:border-stone-800/80 truncate">
                            {{ auth()->user()->email }}
                        </div>
                        <button
                            @click="profileMenuOpen = false; window.dispatchEvent(new CustomEvent('open-settings-ui', { detail: 'general' }));"
                            class="flex items-center justify-between w-full px-3 py-1.5 text-[14px] font-medium text-gray-700 dark:text-stone-200 hover:bg-[#F3F2EE] dark:hover:bg-[#3A3A38] cursor-pointer transition-colors rounded-lg mx-0.5"
                        >
                            <span class="flex items-center gap-2">
                                <svg class="w-4 h-4 text-gray-500 dark:text-stone-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                            ...
                            <span class="text-[12px] text-gray-400 font-normal">Ctrl ⇧ ,</span>
                        </button>
                        <div x-data="{ languageOpen: false, currentLang: (localStorage.getItem('rynude_lang') || 'English').replace('Bahasa Indonesia', 'Indonesia') }" class="relative">
                            <button
                            ...
                            >
                                ...
                                <span class="flex items-center gap-1.5">
                                    <span class="text-[12px] font-medium text-gray-500 dark:text-stone-400" x-text="currentLang"></span>
                                    <svg class="w-3.5 h-3.5 text-gray-400 dark:text-stone-500 transition-transform duration-200" :class="languageOpen ? 'rotate-90' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            ...
                            <div
                                ...
                                x-transition:enter-start="opacity-0 -translate-y-1"
                                x-transition:enter-end="opacity-100 translate-y-0"
                                class="pl-7 pr-3 pb-1 space-y-0.5"
                            >
                                <template x-for="lang in ['English', 'Indonesia']" :key="lang">
                                    <button
                                        @click="currentLang = lang; localStorage.setItem('rynude_lang', lang); languageOpen = false"
                                        class="flex items-center justify-between w-full px-2.5 py-1 text-[13.5px] font-medium text-gray-600 dark:text-stone-300 hover:bg-[#EAE9E5] dark:hover:bg-[#3A3A38] rounded-md transition-colors"
                                    >
                        ...
                        <div x-data="{ learnMoreOpen: false }" class="relative">
                            ...
                            >
                                ...
                                <svg class="w-3.5 h-3.5 text-gray-400 dark:text-stone-500 transition-transform duration-200" :class="learnMoreOpen ? 'rotate-90' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/>
                                </svg>
                            </button>
                            <div
                                ...
                                class="pl-7 pr-3 pb-1 space-y-0.5"
                            >
                                <a href="https://docs.anthropic.com" target="_blank" rel="noopener" class="flex items-center gap-2 px-2.5 py-1 text-[13px] font-medium text-gray-600 dark:text-stone-300 hover:bg-[#EAE9E5] dark:hover:bg-[#3A3A38] rounded-md transition-colors">
                                    ...
                                    <svg class="w-3.5 h-3.5 text-gray-300 ml-auto flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25"/>
                                    </svg>
                                </a>
                                <a href="https://anthropic.com/blog" target="_blank" rel="noopener" class="flex items-center gap-2 px-2.5 py-1 text-[13px] font-medium text-gray-600 dark:text-stone-300 hover:bg-[#EAE9E5] dark:hover:bg-[#3A3A38] rounded-md transition-colors">
                                    <svg class="w-3.5 h-3.5 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                                ...
                                <a href="https://status.anthropic.com" target="_blank" rel="noopener" class="flex items-center gap-2 px-2.5 py-1 text-[13px] font-medium text-gray-600 dark:text-stone-300 hover:bg-[#EAE9E5] dark:hover:bg-[#3A3A38] rounded-md transition-colors">
                                    <svg class="w-3.5 h-3.5 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="flex items-center justify-between w-full px-3 py-1.5 text-[14px] font-medium text-gray-700 dark:text-stone-200 hover:bg-[#F3F2EE] dark:hover:bg-[#3A3A38] cursor-pointer transition-colors rounded-lg mx-0.5">
                                <span class="flex items-center gap-2">
                                    <svg class="w-4 h-4 text-gray-500 dark:text-stone-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
    ...
    <div x-show="!open" x-cloak class="h-full w-full flex-1 min-h-0 flex flex-col items-center py-4 px-2 gap-1 bg-transparent">
        <button
            @click="window.dispatchEvent(new CustomEvent('toggle-sidebar'))"
            class="p-2 rounded-lg hover:bg-[#EAE9E5]/60 dark:hover:bg-[#3A3A38] transition-colors text-gray-500 dark:text-stone-200 hover:text-[#2D2825] dark:hover:text-stone-200 w-full flex items-center justify-center mb-4 group relative"
        >
            ...
            <div class="absolute left-[52px] px-2 py-1 bg-stone-900 text-white text-[12px] font-medium rounded-lg shadow-md whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-[100]">
                Expand sidebar
            </div>
        </button>
        ...
        <button
            @click="window.dispatchEvent(new CustomEvent('open-system-update'))"
            class="p-2 rounded-lg mb-1 transition-colors w-full flex items-center justify-center text-blue-600 bg-blue-50 hover:bg-blue-100 dark:text-blue-400 dark:bg-blue-500/10 dark:hover:bg-blue-500/20 relative group"
        >
        ...
        <button @click="activePanel = null; window.dispatchEvent(new CustomEvent('close-artifact-panel')); if (window.innerWidth < 768) { sidebarOpen = false; open = false; }; window.dispatchEvent(new CustomEvent('newChat')); window.history.pushState({}, '', '{{ route('chat') }}');" class="p-2 rounded-lg hover:bg-[#EAE9E5]/60 dark:hover:bg-[#3A3A38] transition-colors text-gray-500 dark:text-stone-200 hover:text-[#2D2825] dark:hover:text-stone-200 w-full flex items-center justify-center relative group">
            <div class="flex items-center justify-center rounded-full size-[1.4rem] -mx-[0.2rem] border border-[#DCD7D2] dark:border-stone-600 bg-[#EAE6E1]/40 dark:bg-stone-700/30 transition-all ease-in-out group-hover:-rotate-3 group-hover:scale-110 group-active:rotate-6 group-active:scale-[0.98]">
                <svg class="w-[14px] h-[14px] text-gray-600 dark:text-stone-200 group-hover:text-[#2D2825] dark:group-hover:text-white transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
                    <path d="M12 5v14M5 12h14"/>
                </svg>
        ...
        <button
            @click="activePanel = activePanel === 'chats' ? null : 'chats'; window.dispatchEvent(new CustomEvent('close-artifact-panel'))"
            class="p-2 rounded-lg transition-colors w-full flex items-center justify-center relative group"
            :class="activePanel === 'chats' ? 'bg-[#EAE9E5] dark:bg-[#2C2C2C] text-[#2D2825] dark:text-stone-200' : 'text-gray-500 dark:text-stone-200 hover:bg-[#EAE9E5]/60 dark:hover:bg-[#3A3A38] hover:text-[#2D2825] dark:hover:text-stone-200'"
        >
        ...
        <button
            @click="activePanel = activePanel === 'projects' ? null : 'projects'; window.dispatchEvent(new CustomEvent('close-artifact-panel'))"
            class="p-2 rounded-lg transition-colors w-full flex items-center justify-center relative group"
            :class="activePanel === 'projects' ? 'bg-[#EAE9E5] dark:bg-[#2C2C2C] text-[#2D2825] dark:text-stone-200' : 'text-gray-500 dark:text-stone-200 hover:bg-[#EAE9E5]/60 dark:hover:bg-stone-800/50 hover:text-[#2D2825] dark:hover:text-stone-200'"
        >
            <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
                <path class="project-arrow" stroke-linecap="round" stroke-linejoin="round" d="M12 10.5v6.75m0 0l-3-3m3 3l3-3" />
        ...
        <button
            @click="activePanel = activePanel === 'artifacts' ? null : 'artifacts'; window.dispatchEvent(new CustomEvent('close-artifact-panel'))"
            class="p-2 rounded-lg transition-colors w-full flex items-center justify-center relative group"
            :class="activePanel === 'artifacts' ? 'bg-[#EAE9E5] text-[#2D2825] dark:bg-[#2C2C2C] dark:text-stone-200' : 'text-gray-500 hover:bg-[#EAE9E5]/60 hover:text-[#2D2825] dark:text-stone-200 dark:hover:bg-stone-800/50 dark:hover:text-stone-200'"
        >
        ...
        <button
            @click="activePanel = activePanel === 'customize' ? null : 'customize'; window.dispatchEvent(new CustomEvent('close-artifact-panel')); if(activePanel === 'customize') { sidebarOpen = false; open = false; }"
            class="p-2 rounded-lg transition-colors w-full flex items-center justify-center relative group"
            :class="activePanel === 'customize' ? 'bg-[#EAE9E5] text-[#2D2825] dark:bg-[#2C2C2C] dark:text-stone-200' : 'text-gray-500 hover:bg-[#EAE9E5]/60 hover:text-[#2D2825] dark:text-stone-200 dark:hover:bg-stone-800/50 dark:hover:text-stone-200'"
        >
        ...
        <button
            @click="activePanel = activePanel === 'api-keys' ? null : 'api-keys'; window.dispatchEvent(new CustomEvent('close-artifact-panel'))"
            class="p-2 rounded-lg transition-colors w-full flex items-center justify-center relative group"
            :class="activePanel === 'api-keys' ? 'bg-[#EAE9E5] text-[#2D2825] dark:bg-[#2C2C2C] dark:text-stone-200' : 'text-gray-500 dark:text-stone-200 hover:bg-[#EAE9E5]/60 dark:hover:bg-stone-800/50 hover:text-[#2D2825] dark:hover:text-stone-200'"
        >
            <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
        ...
        <a
            href="{{ route('code') }}"
            class="p-2 rounded-lg transition-colors w-full flex items-center justify-center text-gray-500 dark:text-stone-200 hover:bg-[#EAE9E5]/60 dark:hover:bg-stone-800/50 hover:text-[#2D2825] dark:hover:text-stone-200 relative group"
        >
            <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                <path class="code-bracket-right" stroke-linecap="round" stroke-linejoin="round" d="M17.25 6.75L22.5 12l-5.25 5.25"/>
                <path class="code-bracket-left" stroke-linecap="round" stroke-linejoin="round" d="M6.75 17.25L1.5 12l5.25-5.25"/>
        ...
        <button
            @click="activePanel = activePanel === 'cowork' ? null : 'cowork'; window.dispatchEvent(new CustomEvent('close-artifact-panel'))"
            class="p-2 rounded-lg transition-colors w-full flex items-center justify-center relative group"
            :class="activePanel === 'cowork' ? 'bg-[#EAE9E5] dark:bg-[#2C2C2C] text-[#2D2825] dark:text-stone-200' : 'text-gray-500 dark:text-stone-200 hover:bg-[#EAE9E5]/60 dark:hover:bg-stone-800/50 hover:text-[#2D2825] dark:hover:text-stone-200'"
        >
        ...
        @auth
            <div x-data="{ profileMenuOpen: false }" class="relative w-full flex justify-center">
                ...
                <div
                    ...
                    @click.away="profileMenuOpen = false"
                    class="absolute bottom-full mb-2 w-[260px] max-h-[60vh] overflow-y-auto overflow-x-hidden custom-scrollbar bg-[#FCFBFA] dark:bg-[#2C2C2C] border border-gray-200 dark:border-stone-700 rounded-xl shadow-xl z-50 py-1"
                    style="left: 56px; display: none;"
                >
                    <div class="text-[13px] text-gray-700 dark:text-stone-200 font-semibold px-3 py-1 mb-0.5 border-b border-gray-100 dark:border-stone-800/80 truncate text-left">
                        {{ auth()->user()->email }}
                    </div>
                    <button
                        @click="profileMenuOpen = false; window.dispatchEvent(new CustomEvent('open-settings-ui', { detail: 'general' }));"
                ...
                <button
                    ...
                    class="p-1.5 hover:bg-[#EAE9E5]/60 dark:hover:bg-[#3A3A38] rounded-xl transition-colors w-full flex items-center justify-center mb-2 relative group"
                >
                    <svg class="w-8 h-8 flex-shrink-0" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <circle cx="8" cy="8" r="8" fill="#5C92D1"/>
                        <rect x="6" y="6" width="4" height="4" fill="#DBE0ED"/>
                    ...
                    <div class="absolute left-[52px] px-2.5 py-1 bg-stone-900 text-white text-[12px] font-medium rounded-lg shadow-md whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-[100]">
                        {{ auth()->user()->name }}
                    </div>
                </button>
            </div>
        @else
            <a href="{{ route('login') }}" class="p-2 rounded-lg hover:bg-[#EAE9E5]/60 dark:hover:bg-stone-800/50 transition-colors text-gray-500 dark:text-stone-200 hover:text-[#2D2825] dark:hover:text-stone-200 w-full flex items-center justify-center mb-2 relative group">
                <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9"/>
                </svg>
                <div class="absolute left-[52px] px-2.5 py-1 bg-stone-900 text-white text-[12px] font-medium rounded-lg shadow-md whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-[100]">
