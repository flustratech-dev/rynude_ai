<div class="h-full bg-[#F9F8F6] dark:bg-claude-bg-dark flex flex-col font-sans overflow-hidden"
    ...
    <div class="w-full border-b border-[#E5E5E5] dark:border-stone-800/80 px-6 py-3 flex items-center justify-between flex-shrink-0">
        <div class="flex items-center gap-2">
            ...
            <span class="text-[9px] font-semibold tracking-wider uppercase px-1.5 py-0.5 bg-[#EAE9E5] dark:bg-claude-bg-dark text-stone-600 dark:text-stone-400 rounded-[4px]">Beta</span>
        </div>
    ...
    <div class="flex-1 overflow-y-auto px-6 py-5">
        ...
        <template x-if="view === 'landing'">
            <div class="h-full flex flex-col items-center justify-center px-6 py-6">
                <div class="max-w-3xl w-full flex flex-col items-center justify-center">
                    <div class="w-full max-w-[560px] aspect-[1.6] rounded-3xl border border-stone-200 dark:border-stone-800 overflow-hidden shadow-[0_8px_30px_rgba(0,0,0,0.02)] bg-white dark:bg-claude-bg-dark flex items-center justify-center relative">
        ...
        <template x-if="view === 'list'">
            <div>
                <div class="grid grid-cols-4 gap-3 mb-5">
                    <div class="bg-white dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-800 rounded-xl p-3.5 shadow-sm">
                ...
                <div class="flex items-center gap-1 mb-4">
                    <template x-for="(label, key) in {all:'All',pending:'Pending',in_progress:'Running',completed:'Completed',failed:'Failed'}">
                        <button @click="statusFilter = key; loadTasks()" class="px-3 py-1 rounded-lg text-[12.5px] transition-all" :class="statusFilter === key ? 'bg-[#EAE9E5] text-[#2D2825] dark:bg-claude-bg-dark dark:text-stone-200 font-semibold' : 'text-stone-500 hover:text-[#2D2825] dark:hover:text-stone-200'" x-text="label"></button>
                    </template>
                </div>
                ...
                <template x-if="!loading && tasks.length === 0">
                    <div class="bg-white dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-800 rounded-2xl py-14 flex flex-col items-center justify-center text-center shadow-sm">
                ...
                <template x-if="!loading && tasks.length > 0">
                    <div class="space-y-2.5">
                        <template x-for="task in tasks" :key="task.id">
                            <div @click="openTask(task)" class="group bg-white dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-800 rounded-xl px-4 py-3.5 shadow-sm hover:border-stone-300 dark:hover:border-stone-700 hover:shadow transition-all cursor-pointer flex items-center gap-3">
        ...
        <template x-if="view === 'create'">
            <div>
                ...
                <div class="bg-white dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-800 rounded-2xl p-5 shadow-sm space-y-4">
                    <div>
                        ...
                        <input type="text" x-model="form.title" placeholder="e.g. Draft a launch announcement"
                            class="w-full px-3 py-2 bg-[#F9F8F6] dark:bg-claude-bg-dark border border-stone-200 dark:border-stone-700 rounded-lg text-[13.5px] text-stone-800 dark:text-stone-200 placeholder-stone-400 focus:outline-none focus:border-[#D97757]">
                    ...
                    <div>
                        ...
                        <textarea x-model="form.description" rows="5" placeholder="Provide context, requirements, and the desired output…"
                            class="w-full px-3 py-2 bg-[#F9F8F6] dark:bg-claude-bg-dark border border-stone-200 dark:border-stone-700 rounded-lg text-[13.5px] text-stone-800 dark:text-stone-200 placeholder-stone-400 focus:outline-none focus:border-[#D97757] resize-none"></textarea>
                    </div>
                    <div class="grid grid-cols-3 gap-3">
                        <div>
                            <label class="block text-[12.5px] font-medium text-stone-600 dark:text-stone-300 mb-1.5">Model</label>
                            <select x-model="form.model" class="w-full px-3 py-2 bg-[#F9F8F6] dark:bg-claude-bg-dark border border-stone-200 dark:border-stone-700 rounded-lg text-[13px] text-stone-800 dark:text-stone-200 focus:outline-none focus:border-[#D97757]">
                        ...
                        <div>
                            ...
                            <select x-model="form.priority" class="w-full px-3 py-2 bg-[#F9F8F6] dark:bg-claude-bg-dark border border-stone-200 dark:border-stone-700 rounded-lg text-[13px] text-stone-800 dark:text-stone-200 focus:outline-none focus:border-[#D97757]">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" x-model="form.scheduledFor" class="w-full px-3 py-2 bg-[#F9F8F6] dark:bg-claude-bg-dark border border-stone-200 dark:border-stone-700 rounded-lg text-[13px] text-stone-800 dark:text-stone-200 focus:outline-none focus:border-[#D97757]">
                        </div>
                    </div>
        ...
        <template x-if="view === 'detail' && activeTask">
            <div>
                ...
                <div x-show="activeTask.description" class="bg-white dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-800 rounded-xl p-4 shadow-sm mb-4">
                ...
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <template x-if="activeTask.status !== 'completed'">
                        <button @click="updateStatus(activeTask.id, 'completed')" class="px-3.5 py-2 border border-stone-200 dark:border-stone-700 text-stone-600 dark:text-stone-300 rounded-lg text-[13px] font-medium hover:bg-stone-50 dark:hover:bg-[#3A3A38] transition-colors">Mark complete</button>
                    </template>
                    <template x-if="activeTask.status === 'completed'">
                        <button @click="updateStatus(activeTask.id, 'pending')" class="px-3.5 py-2 border border-stone-200 dark:border-stone-700 text-stone-600 dark:text-stone-300 rounded-lg text-[13px] font-medium hover:bg-stone-50 dark:hover:bg-[#3A3A38] transition-colors">Reopen</button>
                    </template>
                ...
                <template x-if="activeTask.result">
                    <div class="bg-white dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-800 rounded-xl p-5 shadow-sm">
