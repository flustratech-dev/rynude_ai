            ...
            <div class="px-4 py-3 flex items-center justify-between bg-transparent shrink-0 z-10 relative">
                <div x-data="{ openMenu: false }" class="flex items-center gap-2 max-w-[50%] relative">
                    <template x-if="currentArtifact.language === 'new'">
                        <div>
                            <button @click="openMenu = !openMenu" @click.away="openMenu = false" class="flex items-center gap-1.5 px-2 py-1 -ml-2 rounded-md hover:bg-[#F3F2F1] dark:hover:bg-[#3A3A38] transition-colors text-[14px] font-medium text-[#2D2825] dark:text-stone-200">
                            ...
                            <div x-show="openMenu" x-transition.opacity.duration.200ms x-cloak class="absolute top-full left-0 mt-1 w-48 bg-white dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-700 rounded-xl shadow-lg py-1.5 z-50">
                                <button class="w-full text-left px-3 py-1.5 text-[13px] font-medium text-[#2D2825] dark:text-stone-200 hover:bg-claude-bg-light dark:hover:bg-[#3A3A38] flex items-center gap-2">
                                ...
                                <button class="w-full text-left px-3 py-1.5 text-[13px] font-medium text-[#2D2825] dark:text-stone-200 hover:bg-claude-bg-light dark:hover:bg-[#3A3A38] flex items-center gap-2 mb-1">
                ...
                <div class="absolute left-1/2 -translate-x-1/2 flex items-center bg-[#F3F2F1] dark:bg-stone-700 p-1 rounded-lg">
                    <button @click="activeTab = 'preview'" class="px-3 py-1.5 text-[13px] font-medium rounded-md transition-all duration-200" :class="activeTab === 'preview' ? 'bg-white dark:bg-claude-bg-dark text-[#2D2825] dark:text-stone-200 shadow-sm' : 'text-stone-500 dark:text-stone-400 hover:text-stone-700 dark:hover:text-stone-300'">Preview</button>
                    <button @click="activeTab = 'code'" class="px-3 py-1.5 text-[13px] font-medium rounded-md transition-all duration-200" :class="activeTab === 'code' ? 'bg-white dark:bg-claude-bg-dark text-[#2D2825] dark:text-stone-200 shadow-sm' : 'text-stone-500 dark:text-stone-400 hover:text-stone-700 dark:hover:text-stone-300'">Code</button>
                </div>
                ...
                <div class="flex items-center gap-1.5">
                    <template x-if="currentArtifact.language !== 'new'">
                        <div class="flex items-center gap-1.5">
                            ...
                            <button @click="copyCode()" class="p-1.5 hover:bg-[#F3F2F1] dark:hover:bg-[#3A3A38] rounded-md transition-colors text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-300" title="Copy code">
                            ...
                            <div x-data="{ openDl: false }" class="relative">
                                <button @click="openDl = !openDl" @click.away="openDl = false" class="p-1.5 hover:bg-[#F3F2F1] dark:hover:bg-[#3A3A38] rounded-md transition-colors text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-300 flex items-center gap-0.5" title="Download">
                                ...
                                <div x-show="openDl" x-cloak x-transition.opacity.duration.200ms class="absolute top-full right-0 mt-1 w-52 bg-white dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-700 rounded-xl shadow-lg py-1.5 z-50">
                                    <button @click="openDl = false; downloadPdf()" class="w-full text-left px-3 py-1.5 text-[13px] font-medium text-[#2D2825] dark:text-stone-200 hover:bg-claude-bg-light dark:hover:bg-[#3A3A38] flex items-center gap-2">
                                    ...
                                    <button @click="openDl = false; downloadDocx()" class="w-full text-left px-3 py-1.5 text-[13px] font-medium text-[#2D2825] dark:text-stone-200 hover:bg-claude-bg-light dark:hover:bg-[#3A3A38] flex items-center gap-2">
                                    ...
                                    <button @click="openDl = false; downloadMarkdown()" class="w-full text-left px-3 py-1.5 text-[13px] font-medium text-[#2D2825] dark:text-stone-200 hover:bg-claude-bg-light dark:hover:bg-[#3A3A38] flex items-center gap-2">
                                    ...
                                    <button @click="openDl = false; downloadTxt()" class="w-full text-left px-3 py-1.5 text-[13px] font-medium text-[#2D2825] dark:text-stone-200 hover:bg-claude-bg-light dark:hover:bg-[#3A3A38] flex items-center gap-2">
                                    ...
                                    <button @click="openDl = false; downloadHtml()" class="w-full text-left px-3 py-1.5 text-[13px] font-medium text-[#2D2825] dark:text-stone-200 hover:bg-claude-bg-light dark:hover:bg-[#3A3A38] flex items-center gap-2">
                            ...
                            <button @click="toggleFullscreen()" class="p-1.5 hover:bg-[#F3F2F1] dark:hover:bg-[#3A3A38] rounded-md transition-colors text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-300" title="Fullscreen">
                            ...
                            <template x-if="currentArtifact.id">
                                <div>
                                    <button x-show="!currentArtifact.is_public" @click="publishArtifact(currentArtifact.id)" class="p-1.5 hover:bg-[#F3F2F1] dark:hover:bg-[#3A3A38] rounded-md transition-colors text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-300" title="Publish">
                                    ...
                                    <button x-show="currentArtifact.is_public" @click="unpublishArtifact(currentArtifact.id)" class="p-1.5 hover:bg-[#F3F2F1] dark:hover:bg-[#3A3A38] rounded-md transition-colors text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-300" title="Unpublish">
                                    ...
                                    <button x-show="currentArtifact.is_public" @click="publishArtifact(currentArtifact.id)" class="p-1.5 hover:bg-[#F3F2F1] dark:hover:bg-[#3A3A38] rounded-md transition-colors text-green-600 dark:text-green-500" title="Published — copy link">
                    ...
                    <button @click="closeArtifact()" class="p-1.5 hover:bg-[#F3F2F1] dark:hover:bg-[#3A3A38] rounded-md transition-colors text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-300" title="Close">
                    ...
                    <div class="flex-1 min-h-0 flex flex-col items-center justify-center p-8 overflow-y-auto custom-scrollbar">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-[800px] w-full">
                            <template x-for="card in [{label:'Apps and websites',icon:'web'},{label:'Documents and templates',icon:'doc'},{label:'Games',icon:'game'},{label:'Productivity tools',icon:'tool'},{label:'Creative projects',icon:'creative'},{label:'Quiz or survey',icon:'quiz'},{label:'Start from scratch',icon:'plus'}]" :key="card.label">
                                <button @click="generateTemplate(card.label)" class="bg-[#FCFBFA] dark:bg-[#323232] border border-[#E5E5E5] dark:border-stone-700 rounded-2xl p-4 aspect-[4/3] flex flex-col items-start justify-between hover:border-[#D97757] hover:shadow-sm transition-all group">
                ...
                <template x-if="currentArtifact.language !== 'new'">
                    <div class="flex-1 min-h-0 overflow-hidden flex flex-col relative">
                                    ...
                                    <div class="flex-1 min-h-0 h-full w-full overflow-y-auto custom-scrollbar bg-stone-100 dark:bg-claude-bg-dark" x-data="pdfViewer('/artifact/' + currentPdfArtifactId + '/preview.pdf')">
                                        <div class="w-full min-h-full py-8 flex flex-col items-center">
                                            <div x-ref="container" class="relative w-full max-w-[210mm] flex flex-col items-center gap-6">
                                                {{-- Loading Spinner --}}
                                                <div x-show="loading" class="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-claude-bg-dark/80 z-10">
                        ...
                        <template x-if="versions.length > 1">
                            <div class="absolute bottom-4 right-4 flex items-center bg-white dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-700 rounded-lg shadow-sm p-1 z-20">
            ...
            <div class="max-w-4xl w-full flex-1 flex flex-col">
                    ...
                    <div class="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 pb-12">
                        <template x-for="artifact in filteredArtifacts" :key="artifact.id">
                            <div class="relative group" x-data="{ menuOpen: false }">
                                ...
                                <button @click.stop="menuOpen = !menuOpen" @click.away="menuOpen = false" class="absolute top-3 right-3 p-1.5 text-stone-400 hover:text-stone-600 dark:hover:text-stone-300 rounded-lg hover:bg-stone-100 dark:hover:bg-[#3A3A38] transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100 z-10" title="Options">
                                ...
                                <div x-show="menuOpen" x-cloak class="absolute top-10 right-3 w-40 bg-white dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-700 rounded-xl shadow-lg py-1.5 z-20">
                                    <button @click.stop="menuOpen = false; renameArtifact(artifact)" class="w-full text-left px-3 py-1.5 text-[13px] font-medium text-[#2D2825] dark:text-stone-200 hover:bg-claude-bg-light dark:hover:bg-[#3A3A38] flex items-center gap-2">
                                        <svg class="w-4 h-4 text-stone-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg> Rename
                                    </button>
                ...
                <template x-if="!loading && filteredArtifacts.length === 0 && searchQuery">
                    <div class="flex-1 flex flex-col items-center justify-center text-center pb-20">
                        <div class="w-12 h-12 rounded-2xl bg-[#F3F2F1] dark:bg-claude-bg-dark flex items-center justify-center text-stone-400 mb-4">
                ...
                <template x-if="!loading && filteredArtifacts.length === 0 && !searchQuery">
                    <div class="flex-1 flex flex-col items-center justify-center text-center pb-20">
                        ...
                        <button @click="createNewArtifact()" class="bg-white hover:bg-gray-50 text-[#2D2825] dark:bg-claude-bg-dark dark:hover:bg-[#3A3A38] dark:text-stone-200 dark:border-stone-600 border border-[#E5E5E5] px-3.5 py-1.5 rounded-lg text-sm font-medium transition-colors shadow-sm">New artifact</button>
                    </div>
                </template>
            </div>
        </div>
