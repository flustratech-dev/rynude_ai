<div class="flex w-full h-full bg-[#F9F8F6] dark:bg-claude-bg-dark" x-data="{
        ...
        <template x-if="activeTab === 'dashboard'">
            <div class="max-w-[600px] w-full flex flex-col items-center">
                ...
                <div class="w-full space-y-4">
                    <button class="w-full flex items-center gap-5 p-5 bg-[#FCFBFA] dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-800 rounded-2xl hover:shadow-md hover:border-stone-300 transition-all text-left group">
                    ...
                    <button @click="setTab('create_skill')" class="w-full flex items-center gap-5 p-5 bg-[#FCFBFA] dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-800 rounded-2xl hover:shadow-md hover:border-stone-300 transition-all text-left group">
                        <div class="w-10 h-10 rounded-xl bg-stone-100 dark:bg-stone-700 flex items-center justify-center text-stone-600 dark:text-stone-300 group-hover:bg-stone-200 dark:group-hover:bg-stone-600 transition-colors shrink-0">
                    ...
                    <button @click="setTab('templates')" class="w-full flex items-center gap-5 p-5 bg-[#FCFBFA] dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-800 rounded-2xl hover:shadow-md hover:border-stone-300 transition-all text-left group">
                        <div class="w-10 h-10 rounded-xl bg-stone-100 dark:bg-stone-700 flex items-center justify-center text-stone-600 dark:text-stone-300 group-hover:bg-stone-200 dark:group-hover:bg-stone-600 transition-colors shrink-0">
            ...
            <div class="max-w-[700px] w-full flex flex-col h-full items-start justify-start py-8">
                <div class="flex items-center justify-between w-full mb-8">
                    ...
                    <div class="flex items-center gap-2">
                        <button @click="setTab('templates')" class="px-4 py-2 bg-[#FCFBFA] dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-700 text-[#2D2825] dark:text-stone-200 hover:bg-stone-50 dark:hover:bg-[#3A3A38] rounded-xl text-sm font-medium transition-colors shadow-sm">
                            Templates
                        </button>
                ...
                @if(count($skills ?? []) > 0)
                    <div class="w-full space-y-3">
                        @foreach($skills as $skill)
                            <div class="w-full p-4 bg-[#FCFBFA] dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-700 rounded-xl shadow-sm flex items-center justify-between">
                ...
                @else
                    <div class="flex-1 flex flex-col items-center justify-center text-center w-full py-16">
                        ...
                        <button @click="setTab('create_skill')" class="px-4 py-2 bg-[#FCFBFA] dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-700 text-[#2D2825] dark:text-stone-200 hover:bg-stone-50 dark:hover:bg-[#3A3A38] rounded-xl text-sm font-medium transition-colors shadow-sm">
                            Create new skill
                        </button>
                    </div>
                @endif
            ...
            <div class="max-w-[760px] w-full flex flex-col h-full items-start justify-start py-8">
                ...
                <div class="w-full grid grid-cols-1 sm:grid-cols-2 gap-4">
                    @foreach($skillTemplates ?? [] as $index => $template)
                        <div class="p-5 bg-[#FCFBFA] dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-700 rounded-2xl shadow-sm flex flex-col">
            ...
            <div class="max-w-[700px] w-full flex flex-col h-full items-start justify-start py-8">
                ...
                <form class="w-full space-y-5" x-data="{
                    ...
                    <div>
                        ...
                        <input type="text" x-model="skillName" placeholder="e.g. Code Reviewer" class="w-full px-3 py-2 bg-[#FCFBFA] dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-700 rounded-lg text-sm text-[#2D2825] dark:text-stone-200 focus:outline-none focus:ring-0 focus:border-stone-300 dark:focus:border-stone-500" required>
                    </div>

                    <div>
                        <label class="block text-[13px] font-medium text-[#2D2825] dark:text-stone-300 mb-1.5">Description (optional)</label>
                        <input type="text" x-model="skillDescription" placeholder="What does this skill do?" class="w-full px-3 py-2 bg-[#FCFBFA] dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-700 rounded-lg text-sm text-[#2D2825] dark:text-stone-200 focus:outline-none focus:ring-0 focus:border-stone-300 dark:focus:border-stone-500">
                    </div>

                    <div>
                        <label class="block text-[13px] font-medium text-[#2D2825] dark:text-stone-300 mb-1.5">Instructions *</label>
                        <textarea x-model="skillInstructions" rows="6" placeholder="You are an expert code reviewer. When I provide code, you should..." class="w-full px-3 py-2 bg-[#FCFBFA] dark:bg-claude-bg-dark border border-[#E5E5E5] dark:border-stone-700 rounded-lg text-sm text-[#2D2825] dark:text-stone-200 focus:outline-none focus:ring-0 focus:border-stone-300 dark:focus:border-stone-500" required></textarea>
                    </div>
