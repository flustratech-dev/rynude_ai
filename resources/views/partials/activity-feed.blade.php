{{--
    Phase 2 (Blade Template Migration) — Activity Feed presentation.

    Livewire-independent partial: it renders the live agent activity stream using
    only Alpine.js state and browser CustomEvents (`agent-stream-event`,
    `agent-generation-start`). It does NOT reference the Livewire `$wire` bridge —
    the previous `$wire.workflowId` lookups are replaced by a Blade-injected
    `workflowId` baked into the Alpine component at render time.

    Inputs (plain PHP values, no Livewire required):
      - $events     array   initial events to seed the feed (default [])
      - $workflowId ?string only render events for this workflow (default null = all)

    Today this is included by the thin Livewire wrapper (livewire/activity-feed),
